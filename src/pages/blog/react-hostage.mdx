...
> Hooks created a lot of problems that simply do not exist in idiomatic JS; Difficult to optimize; Too heavily invested in runtime semantics which makes it difficult to evolve in a direction that make more use of compilers.
> 
> <cite>Evan You ([tweet](https://twitter.com/youyuxi/status/1621873050995261440))</cite>
...
> (thread)
>
> <cite>Dan Abramov ([tweet](https://twitter.com/dan_abramov/status/1093694465917751298))</cite>
...
I do geniunely think Hooks brought both these improvements. While it's not universally agreed upon, I'm in the firm "composability over inheritance" camp and I think sharing behavior via functions is a drastic improvement over class inheritance. As for readability, the length of your code might not be directly correlated with legibility (see [Regex](/blog/perfect-rgb-regex), [code golfing](https://www.dwitter.net/)), React Hooks improve "[Locality of Behavior](https://htmx.org/essays/locality-of-behaviour/)".

Your eyes do less scanning in smaller components. Event listeners, state transformations, and rendered output can have your eyes jumping up and down. React Hooks improve on this. I find function components both quicker and easier to read.

# Readability v.s. Complexity

But readability does not itself go hand-in-hand with complexity. Hooks lowered complexity via localizing behavior, but increased it via the abstractions it had to make.
...
It's not really that memoization never solves problems, but that it can feel frustrating to think about when and where to memoize. The *ergonomics* are poor.
...
>
> <cite>Tanner Linsley ([tweet](https://twitter.com/tannerlinsley/status/1504854824952610818))</cite>
...
React was great when it launched! But there are better options now; almost objectively so.
